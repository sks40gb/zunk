...
<%
 String answer="";
  try{
   ...
   Connection con=con=com.elearn.db.connection.ElearnConnection.getConnection();
   PreparedStatement ps1=con.prepareStatement("update std_result_info set std_ans=?,status=? where question_id=? and std_id=?");
	 
   String UserAns=request.getParameter("R");
   if(UserAns==null)
	   UserAns="";
  /* if(request.getParameter("queid")==null)
	   response.sendRedirect("Nextques1.jsp");*/

   ps1.setString(1,UserAns);

   if((request.getParameter("R"))==null||(request.getParameter("R")).equals(""))
	   ps1.setString(2,"NotAttempt");
   else
	   ps1.setString(2,"Attempt");	

   ps1.setString(3,request.getParameter("queid"));
  ...
  //..........................................................................
   ...
   //response.sendRedirect("Nextques1.jsp");
		  ...
		  <META HTTP-EQUIV="refresh" content="30000;URL=InsertExam.jsp">
