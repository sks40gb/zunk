...
public class TableModel extends STable implements Cloneable {
    ...
    public TableModel getModelById() throws SQLException, IllegalArgumentException, IllegalAccessException, Exception {
        try {
            st = getConnection().createStatement();
            String sql = getSelectQuery(getColumnList());
            removeAllCriteria();
            //add id criteria
            for (SColumn column : getIdColumnList()) {
                addCriteria(column);
             }
            sql+=criteria.createCriteria();           
    ...
    public void addCriteria(String fieldName, Object matchTo, CriteriaType type) throws InvalidColumnOption, NoSuchFieldException {
        ...
        SColumn column = getColumnByFieldName(fieldName);
        criteria.addCriteria(column, matchTo,type);
    }
