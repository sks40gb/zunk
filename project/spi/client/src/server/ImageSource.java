/* $Header: /home/common/cvsarea/ibase/dia/src/server/ImageSource.java,v 1.6 2005/03/14 13:10:32 weaston Exp $ */
package server;
...
public class ImageSource implements MessageConstants {
    ...
    // Magic codes in first 2 bytes of file
    ...
    /**
     * Create an ImageSource for given file name.
     * @param fileName the file name.
     * @param pageOffset the relative page in a multi-page TIFF, 0 is first.
     * @param resolution the resolution, as defined in MessageConstants.
     */
    public ImageSource(ImageServerTask task, String fileName, int pageOffset, int resolution)
    throws IOException {
        if (fileName.charAt(0) == '*') {
            Log.print("init for database: "+fileName);
            assert pageOffset == 0;
            initializeForDatabase(task, Integer.parseInt(fileName.substring(4)), resolution);
        } else {
     ...
     public ImageSource(String fileName, int pageOffset, int resolution)
    throws IOException {
        if (fileName.charAt(0) == '*') {
            Log.print("init for database: "+fileName);
            assert pageOffset == 0;
    ...
    private void initializeForDatabase(ImageServerTask task, int pageId, int resolution)
    throws IOException {
        try {
            Connection con = task.getConnection();
            ResultSet rs = con.createStatement().executeQuery(
                "select image from binder_image"
    ...
    private void initializeForFile(String fileName, int pageOffset, int resolution)
    throws IOException {
