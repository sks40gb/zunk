...
public class ManagedPreparedStatement extends PreparedStatementAdapter {
     ...
     */
    public ManagedPreparedStatement(ServerTask task, String sql) throws SQLException {
        this.task = task;
        this.sql = sql;
    ...
    private void prepareInsert() throws SQLException { 
        ...
        sql
          = "insert ignore into changes (table_nbr, id, age)"
            ...
            +"      ,  ?"             // age
            +" from "+tableName
            +"   where "+tableName+"_id >= last_insert_id()";
        ...
        for (int i = 0; i < table.getTargetCount(); i++) {
            ...
            sql
              = "insert ignore into changes (table_nbr, id, age)"
                ...
                +"    and T."+tableName+"_id > last_insert_id()";
            //System.out.println(sql);
            stmtAfter = addStmt(stmtAfter, sql);
        }
        //System.out.println("==========");
    ...
    public int executeUpdate() throws SQLException {
        ...
        if (stmtBefore != null) {
            task.setBinLogEnabled(false);
            updateAge = task.getUpdateAgeForUpdate();
            if (stmtBefore != null) {
