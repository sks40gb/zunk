/* $Header: /home/common/cvsarea/ibase/dia/src/client/DiaResultSet.java,v 1.5.10.1 2006/03/09 12:09:16 nancy Exp $ */
package client;
 ...
 *      ResultSetMetaData getMetaData() -- ONLY FOR getColumnCount
 *      int findColumn(String columnName) -- NOT YET IMPLEMENTED
 ...
 * </pre>
 * <p> -----------------
 * <p> A table of data representing a database result set, which
 ...
 */
public class DiaResultSet implements ResultSet
{
   ...
   /**
     * Create a new ResultSet with the given row length.
     */
   public DiaResultSet(int rowLength)
   {
      _rowLength = rowLength;
   }

   /**
     * Add a row (as an array of strings) to the ResultSet
     */
   ...
   /**
     * Add a row (as an array of strings) to the ResultSet
     */
   public void setHeadings(String[] rowData)
   {
      _headings = rowData;
   }
   ...
   {
      if (_currentRow <= 0 || _currentRow > data.size()) {
         throw new SQLException("no current row");
      }
      String result = ((String[]) data.get(_currentRow - 1))[columnIndex - 1];
   ...
   public long getLong(int columnIndex) throws SQLException
   {
      ...
      try {
         ...
         // strip number off of beginning of string
         int end = 0;
         if (result.length() == 0) {
            return 0;
         }
         ...
         while (end < result.length() && result.charAt(end) >= '0' && result.charAt(end) <= '9') {
            end++;
         }
         if (end == 0 || end == 1 && (result.charAt(0) == '+' || result.charAt(0) == '-')) {
            return 0;
         }
         else {
            return Long.parseLong(result.substring(0, end));
         }
      }
   }
   ...
   {
      return new DiaResultSetMetaData(_rowLength, _headings);
   }
   ...
   {
      return (_currentRow <= 0 && data.size() > 0);
   }
   ...
   {
      return (_currentRow > 0 && _currentRow == data.size());
   }
   ...
   {
      return (_currentRow <= data.size() ? _currentRow : 0);
   }
   ...
   {
      int dataSize = data.size();
      _currentRow = (row < 0 ? dataSize + row + 1 : row);
      if (_currentRow <= 0) {
         _currentRow = 0;
         return false;
      }
      else if (_currentRow > dataSize) {
         row = (dataSize == 0 ? 0 : dataSize + 1);
         return false;
      }
      else {
         return true;
