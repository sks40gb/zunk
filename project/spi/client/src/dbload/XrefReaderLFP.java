/* $Header: /home/common/cvsarea/ibase/dia/src/dbload/XrefReaderLFP.java,v 1.5.8.1 2006/01/10 15:28:16 nancy Exp $ */
package dbload;

import common.Log;

/**
 * Implementation of cross-reference reader for LFP files.
 */
    ...
    private void acceptIM(String[] fields) throws ErrorException {
        ...
        if (fields.length < 5 || fields.length > 6) {
            throw new ErrorException(
                                    "Invalid number of comma-separated fields: "+fields.length);
        }
        ...
        char boundaryChar
        = (boundaryString.length() == 0 ? ' ' : boundaryString.charAt(0));
        ...
        if (subFields[0].trim().startsWith(EXPLICIT_PATH)) {
            ...
            if (subFields.length < 3 || subFields.length > 4) {
                throw new ErrorException(
                                        "Wrong number of semicolon-separated fields in file path");
            }
            ...
            if (subFields.length > 2) {
                throw new ErrorException(
                                        "Too many semicolon-separated fields in file path");
            }
