...
/**
 * Class <code>OutputValidationReportDialog</code> display all the history report of export for the volume. 
 * 
 * @author  anurag
 */
public class OutputValidationReportDialog extends javax.swing.JFrame {

    private static final String COMPLETED = "Finished";
    ...
    private static String GET_RECORDS_BY_PROJECT = "";
    private static String GET_RECORDS_BY_VOLUME = "";
    private static String GET_ALL_RECORDS = "";
    ...
    private static final String OVP_TABLE_NAME = "OutputValidations";
    private static final String EXPORT_TABLE_NAME = "ExportDetails";
    private static final String TSEARCH_TABLE_NAME = "TSearchExportDetails";

    /** Creates new form OutputValidationReportDialog */
    public OutputValidationReportDialog(String tableName) {
        initComponents();        
    ...
    private void initComponents() {
        ...
        exportCheckBox = new javax.swing.JRadioButton();
        tsearchExportCheckBox = new javax.swing.JRadioButton();
        ...
        jPanel1.add(refreshTableButton, new java.awt.GridBagConstraints());
        ...
        jPanel1.add(downloadReportButton, gridBagConstraints);
        ...
        exportCheckBox.setText("Export");
        exportCheckBox.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                exportCheckBoxActionPerformed(evt);
            }
        });
        checkBoxPanel.add(exportCheckBox);

        tsearchExportCheckBox.setText("Tsearch Export");
        tsearchExportCheckBox.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                tsearchExportCheckBoxActionPerformed(evt);
            }
        });
        checkBoxPanel.add(tsearchExportCheckBox);

        jPanel2.add(checkBoxPanel);
...
private void formWindowClosing(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowClosing
    ...
    private void loadReport(int loadBy) {
        ...
        switch (loadBy) {
            case 0:
                ...
                task = new TaskExecuteQuery(GET_RECORDS_BY_PROJECT, Integer.toString(projectId));
                break;
            case 1:
                // get record by volume Id
                task = new TaskExecuteQuery(GET_RECORDS_BY_VOLUME, Integer.toString(projectId), Integer.toString(volumeId));
                break;
            default:
                // get all record
                task = new TaskExecuteQuery(GET_ALL_RECORDS);
        }
    ...
    private void loadReport() {
        if (tableName.equals(OVP_TABLE_NAME)) {
            model = new ResultSetTableModel(results, new String[]{
                        "Serial No.", "Project Name", "Volume Name", "Status", "User Name", "Run Date", "File Name", "Validation Result"
                    });
        } else {
            model = new ResultSetTableModel(results, new String[]{
                        "Serial No.", "Project Name", "Volume Name", "Status", "User Name", "Run Date", "File Name"
                    });
        }
        reportTable.setModel(model);
        TableColumn column = null;
        column = reportTable.getColumnModel().getColumn(0);
        column.setPreferredWidth(30); // Output validations Id
        ...
        if (tableName.equals("OutputValidations")) {
            column = reportTable.getColumnModel().getColumn(7);
            column.setPreferredWidth(160); // Validation Result
...
private void reportTableMousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_reportTableMousePressed
    ...
    if (index > -1 && reportTable.getValueAt(index, 3) != null && reportTable.getValueAt(index, 3).toString().trim().equals(COMPLETED)) {
         if (tableName.equals(OVP_TABLE_NAME) && reportTable.getValueAt(index, 7).toString().trim().equals("FAIL") ){
                downloadReportButton.setEnabled(true);
         }else if (tableName.equals(EXPORT_TABLE_NAME) || tableName.equals(TSEARCH_TABLE_NAME)){
              downloadReportButton.setEnabled(true);
         }else{
             downloadReportButton.setEnabled(false);
         }
...
private void exportCheckBoxActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_exportCheckBoxActionPerformed
    tsearchExportCheckBox.setSelected(false);    
    loadTableValues(EXPORT_TABLE_NAME);
}//GEN-LAST:event_exportCheckBoxActionPerformed

private void tsearchExportCheckBoxActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_tsearchExportCheckBoxActionPerformed
    exportCheckBox.setSelected(false);
    loadTableValues(TSEARCH_TABLE_NAME);
}//GEN-LAST:event_tsearchExportCheckBoxActionPerformed

private void loadTableValues(String tablename){    
    this.tableName = tablename;
    GET_RECORDS_BY_PROJECT = tableName + ".getRecordByProject";
    GET_RECORDS_BY_VOLUME = tableName + ".getRecordByVolume";
    GET_ALL_RECORDS = tableName + ".getAllRecords";
    int selProject = projectComboBox.getSelectedIndex();
    int selVol = volumeComboBox.getSelectedIndex();
    if(selVol > -1){
        loadReport(GET_BY_VOLUME);
...
private void deleteButtonActionPerformed(java.awt.event.ActionEvent evt) {                                             
    int selRow = reportTable.getSelectedRow();
    String selectedRowId = reportTable.getValueAt(selRow, 0).toString();
    String deleteQuery = tableName + ".deleteRecord";
    final ClientTask  taskDelete = new TaskExecuteUpdate(deleteQuery, selectedRowId);
    boolean isExecuted = taskDelete.enqueue(this);
    while(! isExecuted){
 ...
 */
    ...
    private void downloadReportFile(String localDir) {
        try {
            if(tableName.equals(TSEARCH_TABLE_NAME)){
                FOLDER_NAME =  "$TSearchExport";
            }else{
                FOLDER_NAME =  "$Export";
            }
            //get the Url connection
            java.net.URL url = new URL(CommonProperties.getUploadFolderPath() + FOLDER_NAME + File.separator + projectName + "_" + volumeName + File.separator + fileName);
            ...
            // Close the streams
            out.close();
            JOptionPane.showMessageDialog(this, "Downloading Completed", "Alert", JOptionPane.INFORMATION_MESSAGE);
        } catch (Exception e) {
            JOptionPane.showMessageDialog(this, "Error occured while downloading the file.", "Error", JOptionPane.ERROR_MESSAGE);
            e.printStackTrace();
        }
    }
    ...
    private javax.swing.JRadioButton exportCheckBox;
    ...
    private javax.swing.JRadioButton tsearchExportCheckBox;
