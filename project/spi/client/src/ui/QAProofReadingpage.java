/* $Header: /home/common/cvsarea/ibase/dia/src/ui/QAProofReadingpage.java,v 1.301.2.22 2006/08/23 19:04:52 nancy Exp $ */
...
import client.TaskGetQAFieldValue;
import client.TaskGoodbye;
import client.TaskQACloseGroup;
...
import client.TaskSaveQACorrectionData;
 ...
 * @author Lexpar
 */
final public class QAProofReadingpage extends JFrame implements MessageConstants {
    ...
    private static Map sampledDocumentList = null;
    private QAGroupModel qamodel = null;
    private Object docTableObj[][];
    private Object summaryTableObj[][];
    ...
    public void setSampledDocumentList(Map sampledDocumentList) {
        this.sampledDocumentList = sampledDocumentList;
    }
    ...
    private QAProofReadingpage binderViewer = null;
    ...
    private JFrame activitySelectionFrame= null;
     ...
     * login.
     */
    public synchronized static QAProofReadingpage getInstance() {
        
        if (freeList.size() == 0) {
            freeList.add(new QAProofReadingpage(0,0,0,null,null,null,null));
        }
        QAProofReadingpage result = (QAProofReadingpage) freeList.remove(0);
        //viewerCount++;
    ...
    public synchronized static QAProofReadingpage getInstance(Map sampledDocumentList, 
            int projectId, int volumeId, int groupNumber,String projectName, 
            String samplingType, QAGroupAssignWindow parent, JFrame activitySelectionFrame) {
        
        QAProofReadingpage.sampledDocumentList = sampledDocumentList;
        QAProofReadingpage.parent = parent;
        if (freeList.size() == 0) {
            freeList.add(new QAProofReadingpage(projectId,volumeId, groupNumber,parent,
                    samplingType, projectName, activitySelectionFrame));
        }
        QAProofReadingpage result = (QAProofReadingpage) freeList.remove(0);
        //viewerCount++;
        //Log.print("@@@SPV.getInstance: "+viewerCount+" (using "+result.viewerNumber+")");
     ...
     */
    public void free() {
        synchronized (QAProofReadingpage.class) {
    ...
    /** Creates new form QAProofReadingpage */
    private QAProofReadingpage(int projectId, int volumeId, int groupNumber, QAGroupAssignWindow parent,
            String samplingType, String projectName, JFrame activitySelectionFrame) {
   ...
   public void setSamplingFields(String samplingFields)
   {
    ...
    private void displayImageForDocument(String batesNumber, int childId, String fieldLevel) {
        
        final ClientTask task;
        final String batesNo = batesNumber;
        
        if("l1".equals(fieldLevel)) {
            task = new TaskExecuteQuery("qapr.getL1PageId", ""+childId);
        }else {
            task = new TaskExecuteQuery("qapr.getPageId", ""+batesNumber, ""+childId);
        }
                    ...
                    public void run() {
                        try {
                               ...
                               if(result.next()) {
                                   ...
                                   int offset = result.getInt(2);
                                   String boundary = result.getString(3);
                                   
                                   int boundaryValue = 0;
                                   if("NONE".equals(boundary)) {
                                       boundaryValue = B_NONE;
                                   }else if("CHILD".equals(boundary)) {
                                       boundaryValue = B_CHILD;
                                   }else if("RANGE".equals(boundary)) {
                                       boundaryValue = B_RANGE;
                                   }
                                   
//                                   getCoderData(pageId, offset, boundaryValue);
                                   getCoderData(pageId, offset, boundaryValue,"QA",""+batesNo,volumeId);
                               }
    ...
    private void setActionMap(JPanel thePane) {
        upKey = midScrollPane.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
                            .get(KeyStroke.getKeyStroke(KeyEvent.VK_UP, 0));
        downKey = midScrollPane.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
                            .get(KeyStroke.getKeyStroke(KeyEvent.VK_DOWN, 0));
        midScrollPane.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
        ...
        pageUpKey = midScrollPane.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
                            .get(KeyStroke.getKeyStroke("PAGE_UP"));
        pageDownKey = midScrollPane.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
                            .get(KeyStroke.getKeyStroke("PAGE_DOWN"));
        midScrollPane.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
                            .put(KeyStroke.getKeyStroke("PAGE_UP"),
                                "doPageUp");
        midScrollPane.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
                            .put(KeyStroke.getKeyStroke("PAGE_DOWN"),
                                "doPageDn");

        thePane.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
                            .put(KeyStroke.getKeyStroke(KeyEvent.VK_N, InputEvent.ALT_MASK),
                                "doSaveAndGo");
        thePane.getActionMap().put("doSaveAndGo", saveAction);
       
        thePane.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
                            .put(KeyStroke.getKeyStroke(KeyEvent.VK_A, InputEvent.ALT_MASK),
                                "doCopyAll");
        thePane.getActionMap().put("doCopyAll", copyAllAction);
       
        thePane.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
                            .put(KeyStroke.getKeyStroke(KeyEvent.VK_C, InputEvent.ALT_MASK),
                                "doCopyField");
        ...
        thePane.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
                            .put(KeyStroke.getKeyStroke(KeyEvent.VK_F1, InputEvent.ALT_MASK),
                                "doF1");
        thePane.getActionMap().put("doF1", vPagePrevAction);
        thePane.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW)
                            .put(KeyStroke.getKeyStroke(KeyEvent.VK_F2, InputEvent.ALT_MASK),
                                "doF2");
        thePane.getActionMap().put("doF2", vPageNextAction);
        thePane.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
                            .put(KeyStroke.getKeyStroke(KeyEvent.VK_F3, InputEvent.ALT_MASK),
                                "doF3");
        thePane.getActionMap().put("doF3", vAttachPrevAction);
        thePane.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
                            .put(KeyStroke.getKeyStroke(KeyEvent.VK_F4, InputEvent.ALT_MASK),
                                "doF4");
        thePane.getActionMap().put("doF4", vAttachNextAction);
        thePane.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
                            .put(KeyStroke.getKeyStroke(KeyEvent.VK_F5, InputEvent.ALT_MASK),
                                "doF5");
        thePane.getActionMap().put("doF5", vDocPrevAction);
        thePane.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
                            .put(KeyStroke.getKeyStroke(KeyEvent.VK_F6, InputEvent.ALT_MASK),
                                "doF6");
        thePane.getActionMap().put("doF6", vDocNextAction);
        thePane.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
                            .put(KeyStroke.getKeyStroke(KeyEvent.VK_F7, InputEvent.ALT_MASK),
                                "doF7");
        thePane.getActionMap().put("doF7", vFirstCurrAction);
        thePane.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
                            .put(KeyStroke.getKeyStroke(KeyEvent.VK_F8, InputEvent.ALT_MASK),
                                "doF8");
        ...
        thePane.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
                            .put(KeyStroke.getKeyStroke(KeyEvent.VK_D, InputEvent.ALT_MASK),
                                "doDoc");
        thePane.getActionMap().put("doDoc", docAction);
        // I for Child
        thePane.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
                            .put(KeyStroke.getKeyStroke(KeyEvent.VK_I, InputEvent.ALT_MASK),
                                "doChild");
        thePane.getActionMap().put("doChild", childAction);
        // X for Clear
        thePane.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
                            .put(KeyStroke.getKeyStroke(KeyEvent.VK_X, InputEvent.ALT_MASK),
                                "doClear");
        thePane.getActionMap().put("doClear", clearAction);
        // Ctl Z for Undo
        thePane.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW)
                            .put(KeyStroke.getKeyStroke(KeyEvent.VK_Z, InputEvent.CTRL_MASK),
                                "doUndo");
        thePane.getActionMap().put("doUndo", getUndoController().getUndoAction());
        // Ctl Y for Redo
        thePane.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW)
                            .put(KeyStroke.getKeyStroke(KeyEvent.VK_Y, InputEvent.CTRL_MASK),
                                "doRedo");
    ...
    public void initializeForProject(String projectName, int projectId, String whichStatus
                                     , String splitDocuments) {

        Log.print("SPV.initializeForProject"
	    ...
	    closeVolume.setVisible(false); 
            ...
            assignListingQCMenuItem.setVisible(false);
            assignTallyQCMenuItem.setVisible(false);
            if (! (Global.mainWindow instanceof QAProofReadingpage)) {
	    ...
	    closeVolume.setVisible(false); 
         ...
         if ("Listing".equals(whichStatus)) {             
             ...
             assignListingQCMenuItem.setVisible(true);             
             assignTallyQCMenuItem.setVisible(false);
	    ...
	    tallyQCDone.setVisible(true); 
             ...
             viewerLabel.setText(" ");
             coderToolBar.setVisible(false);
             setTitle("TallyQC for project :" + getFieldName());
             coderPane.setVisible(false);
                         
             jPanel22.setVisible(false);
            
             closeBatchMenuItem.setVisible(false);
             addTimerPanel("TallyQC");
             //closeMenuItem.setVisible(false);
             assignListingQCMenuItem.setVisible(false);
             assignTallyQCMenuItem.setVisible(false);
             openBinderMenuItem.setVisible(false);
 ...
 //for F10
   ...
   public boolean getVisibleQueryTracker (){
    ...
    private void setUnitizeNavigation() {
        ...
        coderPanel.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
                            .put(KeyStroke.getKeyStroke(KeyEvent.VK_N, InputEvent.CTRL_MASK),
                                "doSaveAndGoToPage");
        coderPanel.getActionMap().put("doSaveAndGoToPage",
                                 savePageAction);

        coderPanel.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
                            .put(KeyStroke.getKeyStroke(KeyEvent.VK_UP, InputEvent.ALT_MASK),
                                "doPrevPageDoc");
        coderPanel.getActionMap().put("doPrevPageDoc",
                                 prevDocPageAction);

        coderPanel.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
                            .put(KeyStroke.getKeyStroke(KeyEvent.VK_DOWN, InputEvent.ALT_MASK),
                                "doNextPageDoc");
    ...
    //                            getProjectFieldsValidationEntry(validationData);
    //                       }
                   //     }
                    } catch (Throwable th) {
                        Log.quit(th);
        ...
        for (int i = 0; i < compCount; i++) {
           ...
           //Log.print("(SPV.initDynamic) " + value.unitize);
            ...
            || value.unitize.equals("Yes") ) {
                ...
                String minmax = null;
                if (value.minValue != null && ! value.minValue.equals("")
                    && value.maxValue != null && ! value.maxValue.equals("")) {
                ...
                if (value.fieldType.equals("date")) {
                    ...
                    getUndoController().registerDocument( ((JTextField)comp).getDocument()
                                                     , (JTextField)comp);
                    ...
                    if (value.tablespecId > 0) {
                       ...
                       System.out.println("theCodingData makeIbaseTextField=================================>>"+theCodingData);
                        comp = makeIbaseTextField(value.fieldType
                            , value.tablespecId, name, value.tableMandatory, value.repeated
                            , value.is_level_field, value.mask, value.validChars, value.invalidChars,value.projectId,value.fieldName,documentNumber,batesNumber,whichStatus,imagePath,childId,batchId,theCodingData);
                      
                        // if(null != value.queryraised && "Yes".equals(value.queryraised) && "Yes".equals(showQueryRaised)){                       
                         if(null != value.queryraised && "Yes".equals(value.queryraised)){                              
                    ...
                    //    dynamicPane.add(0, ++row, comp, "f=H,wx=100");
                        ...
                        if (value.fieldSize > 25) {
                           ...
                           // comp = new beans.LTextButton(value.fieldSize,value.projectId,value.fieldName,theCodingData,whichStatus);
                            ...
                            getUndoController().registerDocument(((beans.LTextButton)comp).getDocument()
                                                                 , ((beans.LTextButton)comp).getTextField());
                        } else {
                            if (! value.mask.equals("")
                                || ! value.validChars.equals("")
                                || ! value.invalidChars.equals("")) {
                                Log.print("(SPV.initDynamic) comp is text LFormatted("+value.fieldSize+"," + value.mask+","+
                                          value.validChars+","+value.invalidChars);
                                comp = new beans.LFormattedTextField(value.fieldSize, value.mask
                                                                     , value.validChars, value.invalidChars,value.projectId,value.fieldName,theCodingData,whichStatus);
                                getUndoController().registerDocument(((beans.LFormattedTextField)comp).getDocument()
                                                                     , (beans.LFormattedTextField)comp);
                        ...
                        Log.print(" back");
                        if("Yes".equals(value.queryraised)){                       
                          JLabel label = new JLabel("F10");
                          label.setFont(new java.awt.Font("Dialog", java.awt.Font.PLAIN, 10));                                          
                          groupPane.add(0, ++row, makeJPanel(comp, label));                             
                        }else{
...
//                             obj.setChildId(child_Id);
                        ...
                        comp = makeIbaseTextField(value.fieldType,
                            value.tablespecId, name, value.tableMandatory, value.repeated
                            , value.is_level_field, value.mask
                            , value.validChars, value.invalidChars,value.projectId,value.fieldName,documentNumber,bates_Number,whichStatus,image_Path,child_Id,batch_Id,theCodingData);
                    ...
                    if("QA".equals(whichStatus) && getSamplingFields() != null && getSamplingFields().trim().length() != 0){ 
                       ...
                       for(String fieldname : samplefield){                                                   
                          if(name.equalsIgnoreCase(fieldname)){
                              theFieldMapper.add(name, (LField) comp, componentCount
                                          , /* has checkbox -> */ true);                         
                          }
                       }
                    }else{
                ...
                } else if (value.fieldGroup != activeGroup
                           && ! unitize) {
        ...
        if (compCount > 0
            ...
            } else if("CodingQC".equals(whichStatus) || "Masking".equals(whichStatus)){
                JPanel p = new JPanel();
                p.add(saveAndGoButton);
                dynamicPane.add(0, ++row, p, "it=12");
    ...
    private beans.IbaseTextField makeIbaseTextField(String type, final int tablespecId, String name
                                                   , String mandatory, String repeated
                                                   , String is_level_field, String mask
                                                   , String validChars, String invalidChars,int projectId,String fieldName,String documentNumber,String batesNumber,String whichStatus,String imagepath,int childId,int batchId,CodingData theCodingData) {
        ...
        if (is_level_field.equals("No")) {    
             System.out.println("childId---------------- "+ childId);
             //System.out.println("theCodingData childId ----------------- "+ theCodingData.childId);
            ...
            area = new beans.IbaseTextField(type, tablespecId
                        ...
                        , ((updateable.equals("CoderAdd")) ? true : false)
                        , ((repeated.equals("Yes")) ? true : false)
                        , theFieldMapper, projectMap, mask, validChars, invalidChars,project_Id,fieldName,documentNumber,batesNumber,whichStatus,imagepath,childId,batchId,theCodingData);
        }
        if (projectId == 0) {
            // ibaseTextField uses a global table -- load it by table_name
            ManagedTableSorter mts = new ManagedTableSorter(1, 0,
                    SQLManagedTableModel.makeInstance("SplitPaneViewer.tablevalueGlobal"
                                                      , tablespecId));
        ...
        // add the undo listener
        getUndoController().registerDocument( ((JTextField)area.getTextField())
                                         .getDocument(), (JTextField)area.getTextField());
    ...
    private class BoundaryDocument extends PlainDocument {
                ...
                throws BadLocationException
        {
            ...
            if (offs > 0
                ...
                Log.print("BEEP> LTextField: insertString(" + offs
                          + ",'" + str + "') " + this.getLength());
            } else if (str.equals(" ")) {
    ...
    private boolean checkFirstPageOfVolume(String str) {
        if (isBoundary(theCodingData, FIRST_PAGE_OF_VOLUME)
            && (str.trim().equals("")
            || str.equalsIgnoreCase("C")))
        {
            // boundary must be document level for first page of region
            Toolkit.getDefaultToolkit().beep();
            Log.print("BEEP> QAProofReadingpage.crossFieldEdit");
            Object[] options = {"Ok"};
    ...
    private void exitApplication() {
        //Log.print("@@@QAProofReadingpage.exitApplication");
        try {
...
//        }
    ...
    public void exitForm(java.awt.event.WindowEvent evt) {                          
        //Log.print("@@@QAProofReadingpage.exitForm");
    ...
    protected boolean crossFieldEdit(boolean force) {        
        ...
        if (unitize) {        
            ...
            if (isBoundary(theCodingData, FIRST_PAGE_OF_VOLUME)
                && (boundaryText.getText().trim().equals("")
                || boundaryText.getText().equalsIgnoreCase("C")))
            {
                // boundary must be document level for first page of region
                Toolkit.getDefaultToolkit().beep();
                Log.print("BEEP> QAProofReadingpage.crossFieldEdit");
        ...
        if (error != null && ! error.equals("")) {
            ...
            if (comp instanceof IbaseTextField
                && ! comp.getText().equals("")) {
	...
	docsLabel = new javax.swing.JLabel();
        sampleFieldsLabel = new javax.swing.JLabel();
        sampleFieldsTextArea = new javax.swing.JTextArea();
	...
	docsPane = new javax.swing.JPanel();
        ...
        closeQAProofReading = new javax.swing.JMenuItem();

        //------------->
        fieldsPane = new javax.swing.JPanel();
        sampleFieldsPane = new javax.swing.JPanel();
        sampleBRPane = new javax.swing.JPanel();
        samplingButtonsPane = new javax.swing.JPanel();
        saveCurrentDocButton = new javax.swing.JButton();
        deleteDocButton = new javax.swing.JButton();
        clearDocButton = new javax.swing.JButton();
        correctionPane = new javax.swing.JPanel();
        correctionDataPane = new javax.swing.JPanel();
        correctionScrollPane = new javax.swing.JScrollPane();
        ...
        docsScrollPane = new javax.swing.JScrollPane();
        ...
        docsTable = new ProjectDocumentTableClass();
        summaryTable = new ProjectFieldsTableClass();
		...
		jLabel11 = new javax.swing.JLabel();
                ...
                correctionDataLabel = new javax.swing.JLabel();
...
//	     newValuetextfield= new javax.swing.JTextField(); 
        ...
        coderPanel.setName("QAProofReadingpage_jPanel1");
        ...
        markingTable.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Start Bate","End Bate","Field Value"
            }
        ) {
            boolean[] canEdit = new boolean [] {
       ...
       // theCoderPane.add(markingPane, java.awt.BorderLayout.LINE_START);
        ...
        sweeppane1.setLayout(new java.awt.FlowLayout(java.awt.FlowLayout.CENTER));  
		
        oldValueLabel.setFont(new java.awt.Font("Dialog", 1, 14));
        oldValueLabel.setText("Old Value:");
        sweeppane1.add(oldValueLabel);
		
        oldValuetextfield.setColumns(15);
		oldValuetextfield.setEnabled(false);
        //oldValuetextfield.setText("2500");
        sweeppane1.add(oldValuetextfield);

        
        sweeppane1.add(sweepButton);

		
        sweeppane1.add(sweepAllButton);
        //theCoderPane.add(sweeppane1, java.awt.BorderLayout.LINE_START);
		 

        sweeppane2.setLayout(new java.awt.FlowLayout(java.awt.FlowLayout.LEFT));    
           
		newValueLabel.setFont(new java.awt.Font("Dialog", 1, 14));
        newValueLabel.setText("New Value:");
        sweeppane2.add(newValueLabel);
		
        newValuetextfield.setColumns(15);
        

        //batchspanField.setText("2500");
        sweeppane2.add(newValuetextfield);
        
        sweeppane2.add(back2Button);
        errorTypePane.setLayout(new java.awt.FlowLayout(java.awt.FlowLayout.LEFT));    
        errorLabel.setFont(new java.awt.Font("Dialog", 1, 14));
        errorLabel.setText("Error Type:");
        errorTypePane.add(errorLabel);
        
		errorComoBox.setPreferredSize(new java.awt.Dimension(200, 25));
		
        errorComoBox.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                //errorComoBoxActionPerformed(evt);
            }
        });
        errorTypePane.add(errorComoBox);  
	...
	docsPane.setLayout(new java.awt.BorderLayout());
        docsPane.setFocusCycleRoot(true);
        docsPane.setPreferredSize(new java.awt.Dimension(238, 250));
	docsLabel.setText("SAMPLED DOCUMENTS");
        docsPane.add(docsLabel,java.awt.BorderLayout.PAGE_START);

        docsScrollPane.setPreferredSize(new java.awt.Dimension(400, 300));
        docsTable.setBorder(new LineBorder(Color.BLACK));   
        
        docsTable.setModel(new javax.swing.table.DefaultTableModel(
                new Object[][]{
        },
                new String[]{
            "Document No", "First Page", "End Page"
        }) {
        ...
        docsTable.setFocusable(false);
        docsTable.setIntercellSpacing(new java.awt.Dimension(2, 1));
        docsTable.addMouseListener(new java.awt.event.MouseAdapter() {

            public void mouseClicked(java.awt.event.MouseEvent evt) {
                docsTableMouseClicked(true);
            }
        });
        
        docsScrollPane.setViewportView(docsTable);

        docsPane.add(docsScrollPane, java.awt.BorderLayout.CENTER);
        
        
        // Pane to display sample fields - added by Prakash
        
        sampleFieldsPane.setLayout(new java.awt.BorderLayout());
        sampleFieldsPane.setFocusCycleRoot(true);
        sampleFieldsPane.setPreferredSize(new java.awt.Dimension(238, 250));
	sampleFieldsLabel.setText("SAMPLED FIELD");
        sampleFieldsTextArea.setPreferredSize(new java.awt.Dimension(238,90));
        //sampleFieldsTextArea.setEnabled(false);
        sampleFieldsTextArea.setBackground(new Color(0,255,255));
        ...
        //sampleFieldsTextArea.setBorder(new javax.swing.border.CompoundBorder(new javax.swing.border.EmptyBorder(new java.awt.Insets(40, 40, 40, 40)), new javax.swing.border.CompoundBorder(new javax.swing.border.BevelBorder(javax.swing.border.BevelBorder.LOWERED), new javax.swing.border.EmptyBorder(new java.awt.Insets(40, 40, 40, 40)))));
        sampleFieldsTextArea.setBorder(new LineBorder(Color.BLACK)); 

        sampleFieldsTextArea.setText("");
        sampleFieldsTextArea.setEditable(false);
        
        correctionPane.setLayout(new java.awt.BorderLayout());
        correctionPane.setFocusCycleRoot(true);
        correctionPane.setPreferredSize(new java.awt.Dimension(400, 250)); // 350
        
        errorPane.setLayout(new java.awt.FlowLayout());
        //errorPane.setFocusCycleRoot(true);
        errorPane.setPreferredSize(new java.awt.Dimension(400, 60));
        
        errorCorrectionLabel.setText("Correction Type :");
        //correctionComboBox.setPreferredSize(new java.awt.Dimension(80,20));
        //correctionCheckBox.setText("View Multiple Entries");
        correctionComboBox.addItem("Added");
        correctionComboBox.addItem("Miscoded");
        correctionComboBox.addItem("Uncoded");
        
        //errorPane.add(correctionCheckBox,java.awt.FlowLayout.LEFT);
        errorPane.add(errorCorrectionLabel,java.awt.FlowLayout.LEFT);
        errorPane.add(correctionComboBox);
        
        
        correctionDataPane.setLayout(new java.awt.BorderLayout());
        correctionDataPane.setFocusCycleRoot(true);
        correctionDataPane.setPreferredSize(new java.awt.Dimension(200, 180));
        
        correctionDataLabel.setText("CORRECTION DATA");
        correctionTextArea.setPreferredSize(new java.awt.Dimension(200,70));
        //sampleFieldsTextArea.setEnabled(false);
        correctionTextArea.setBackground(new Color(204,255,204));
        correctionTextArea.setBorder(new LineBorder(Color.BLACK));
        
        correctionScrollPane.setPreferredSize(new java.awt.Dimension(200, 100));
        
        //****************//
        samplingButtonsPane.setLayout(new java.awt.FlowLayout());
        samplingButtonsPane.setFocusCycleRoot(true);
        samplingButtonsPane.setPreferredSize(new java.awt.Dimension(238, 30));
        saveCurrentDocButton.setText("Save");
        deleteDocButton.setText("Delete");
        clearDocButton.setText("Clear");
        saveCurrentDocButton.setEnabled(false);
        deleteDocButton.setEnabled(false);
        clearDocButton.setEnabled(false);
        correctionTextArea.setEnabled(false);
        
        samplingButtonsPane.add(saveCurrentDocButton);
        samplingButtonsPane.add(deleteDocButton);
        samplingButtonsPane.add(clearDocButton);
        ...
        saveCurrentDocButton.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                saveCurrentDocButtonActionPerformed(evt);
            }
        });
        ...
        correctionDataPane.add(correctionDataLabel,java.awt.BorderLayout.NORTH);
        correctionDataPane.add(correctionTextArea,java.awt.BorderLayout.CENTER);
        correctionDataPane.add(errorPane,java.awt.BorderLayout.SOUTH);
        
        
        correctionPane.add(correctionDataPane,java.awt.BorderLayout.NORTH);
        correctionPane.add(samplingButtonsPane,java.awt.BorderLayout.CENTER);
        //correctionPane.add(correctionScrollPane, java.awt.BorderLayout.SOUTH);
        
        //correctionPane.add(errorCorrectionLabel,java.awt.FlowLayout.CENTER);
        //correctionPane.add(correctionComboBox,java.awt.FlowLayout.RIGHT);
        
        sampleFieldsPane.add(sampleFieldsLabel,java.awt.BorderLayout.NORTH);
        sampleFieldsPane.add(sampleFieldsTextArea,java.awt.BorderLayout.CENTER);
        sampleFieldsPane.add(correctionPane,java.awt.BorderLayout.SOUTH);
                
        // commented by Prakash
        sampleBRPane.setLayout(new java.awt.BorderLayout());
        sampleBRPane.setFocusCycleRoot(true);
        sampleBRPane.setPreferredSize(new java.awt.Dimension(238, 350));
        
        
        
        summaryScrollPane.setPreferredSize(new java.awt.Dimension(200, 200)); //180
        summaryTable.setBorder(new javax.swing.border.SoftBevelBorder(javax.swing.border.BevelBorder.RAISED));     
        
        summaryTable.setModel(new javax.swing.table.DefaultTableModel(
                new Object[][]{
        },
                new String[]{
            "FieldName", "FieldData", "CorrectionData", "CorrectionType"
        }) {
        ...
        summaryTable.setBackground(new Color(0,255,255));
        summaryTable.setFocusable(false);
        summaryTable.setIntercellSpacing(new java.awt.Dimension(2, 1));
        ...
        sampleBRPane.add(summaryScrollPane, java.awt.BorderLayout.NORTH);
//        
//        
        sampleBRPane.add(coderToolBar, new java.awt.BorderLayout().SOUTH);
        ...
        topPane.setPreferredSize(new java.awt.Dimension(250, 150));
	...
	theCoderPane.add(docsPane, java.awt.BorderLayout.PAGE_START);
        theCoderPane.add(sampleFieldsPane,java.awt.BorderLayout.CENTER); 
        theCoderPane.add(sampleBRPane,java.awt.BorderLayout.SOUTH);
        ...
        closeQAProofReading.setText("Close");
        closeQAProofReading.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                closeQAProofReadingActionPerformed(evt);
            }
        });

        closeMenuItem.setMnemonic('C');
        closeMenuItem.setText("Close Group");
        ...
        fileMenu.add(closeQAProofReading);
    ...
    private void generatePayrollSummaryTeam(String[] payPeriod) {
        AbstractReport currentReport = new MenuReport(
            "report_payroll_summary_team", "Payroll for Team " + team_name
            , "Start Date", payPeriod[0], payPeriod[1]
            , "End Date", payPeriod[2], payPeriod[3]);
    ...
    private void generatePayrollSummarySelf(String[] payPeriod) {
        AbstractReport currentReport = new MenuReport(
            "report_payroll_summary_user", "Payroll Summary for " + user_name
            , "Start Date", payPeriod[0], payPeriod[1]
            , "End Date", payPeriod[2], payPeriod[3]);
        currentReport.generate();
    }
    ...
    private void generatePayrollTotalsSelf(String[] payPeriod) {
        AbstractReport currentReport = new MenuReport(
            "report_payroll_detail_user", "Payroll for " + user_name
            , "Start Date", payPeriod[0], payPeriod[1]
            , "End Date", payPeriod[2], payPeriod[3]);
        currentReport.generate();
    }
    ...
    private void generatePayrollTotals(String[] payPeriod) {
        AbstractReport currentReport = new MenuReport(
            "report_payroll_detail_team", "Payroll for Team " + team_name
            , "Start Date", payPeriod[0], payPeriod[1]
            , "End Date", payPeriod[2], payPeriod[3]);
        currentReport.generate();
    }
    ...
    private void editMenuMenuSelected(javax.swing.event.MenuEvent evt) {                                      
        ...
        && theImageData != null
        && theCodingData.childId == theImageData.childId
        && ("Coding".equals(whichStatus) || "CodingQC".equals(whichStatus)
            || "QA".equals(whichStatus) || "Admin".equals(whichStatus))
        && splitDocuments)
        {
    ...
    public void saveView() {
        ...
        if (viewPrefix == null) {
            ...
            if (currentView == CODING_UNDOCKED) {
                position += ","+undockedDialog.getX()+","+undockedDialog.getY()
                                  +","+undockedDialog.getWidth()+","+undockedDialog.getHeight();
            } else {
            ...
            task.setCallback(new Runnable() {
                    public void run(){
                        try {
                            ...
                            Log.print("open binder b="+binderBatchId
                                      +" v="+binderVolumeId+" p="+project);
                            binderViewer = QAProofReadingpage.getInstance();
                            ...
                            binderViewer.initializeForProject(// batchNumber,
                                                        ...
                                                        /* whichStatus=> */ "Binder",
                                                        /* splitDocuments=> */ "No");
                            //binderViewer.setParent(QAProofReadingpage.this);
                            // Open the viewer.
    ...
    private void removeFromBinderItemActionPerformed(java.awt.event.ActionEvent evt) {                                                     
        try {
            ...
            final int nextRelativeChild
                 = theCodingData.batchChildCount <= 1
                   ? 0
            ...
            final ClientTask task
                    = new TaskBinderUpdate(theCodingData.pageId, /* remove => */ true);
            task.setCallback(new Runnable() {
                    public void run() {
                        try {
    ...
    private void addToBinderItemActionPerformed(java.awt.event.ActionEvent evt) {                                                
        try {
            if (saveDataCheck() && crossFieldEdit()) {
                final ClientTask task
                        = new TaskBinderUpdate(theCodingData.pageId, /* remove => */ false);
    ...
    private void boundaryMenuMouseEntered(java.awt.event.MouseEvent evt) {                                          
        try {
            if (! unitize
            && ! whichStatus.equals("Binder")) {
                if (theImageData.childId != theCodingData.childId) {
                    documentMenuItem.setEnabled(false);
                    childMenuItem.setEnabled(false);
                    clearMenuItem.setEnabled(false);
                } else if("Listing".equals(whichStatus)|| "ListingQC".equals(whichStatus)
                    ||"Tally".equals(whichStatus) || "TallyQC".equals(whichStatus)){
                    if (! isBoundary(theImageData, FIRST_PAGE_OF_CHILD)) {
                        documentMenuItem.setEnabled(true);
                        childMenuItem.setEnabled(true);
                        clearMenuItem.setEnabled(false);
                    } else if (theImageData.boundaryFlag.equals("C")) {
                        // child
                        documentMenuItem.setEnabled(true);
                        childMenuItem.setEnabled(false);
                        ...
                        documentMenuItem.setEnabled(false);
                        childMenuItem.setEnabled(! isBoundary(theImageData,
    ...
    private void closeMenuItemActionPerformed(java.awt.event.ActionEvent evt) {
            ...
            ClientTask task = new TaskQACloseGroup(groupNumber, volumeId);
            task.setCallback(new Runnable() {
        ...
        parent.setQAGroupValues();
        parent.fillQAGroupTable();
        dispose();
	...
	private void assignTallyQCMenuItemActionPerformed(java.awt.event.ActionEvent evt) {                                                      
      ...
      private void closeVolumeActionPerformed(java.awt.event.ActionEvent evt) {                                            
        try {         
                final ClientTask task = new TaskCloseVolume(volumeId,whichStatus,0);
                task.setCallback(new Runnable() {
                        public void run() {    
                           ...
                           // final ResultSet rs = Sql.resultFromXML(reply);
                            ...
                            if (T_FAIL.equals(action)) {
                                JOptionPane.showMessageDialog(QAProofReadingpage.this,
                                        " open Error Dialog",
                                        "Error",
                                        JOptionPane.ERROR_MESSAGE);
                            }else{
     ...
     * @see     ui.QAProofReadingpage.copyAll
     */
    ...
    private void closeQAProofReadingActionPerformed(java.awt.event.ActionEvent evt) { 
     
        ClientTask taskEvent = new TaskCloseEvent(CommonConstants.PROCESS_QA, volumeId);
        taskEvent.enqueue(this);
        setVisible(false);
        dispose();
        if(null != activitySelectionFrame) {
            activitySelectionFrame.setVisible(true);
        }
    }    
    ...
    private void vZoomWindowButtonActionPerformed(java.awt.event.ActionEvent evt) {                                                  
        //Log.print("(QAProofReadingpage).vZoomWindowButtonActionPerformed ---");
    ...
    private void absoluteButtonActionPerformed(java.awt.event.ActionEvent evt) {                                               
        try {
            ...
            if ( saveDataCheck() ) {
                SelectRelativeDialog selectDialog = new SelectRelativeDialog(null
                         , String.valueOf(theCodingData.batchChildPosition)
                         , theCodingData.batchChildCount);
                selectDialog.setModal(true);
                selectDialog.show();
    ...
    private void docLastButtonActionPerformed(java.awt.event.ActionEvent evt) {                                              
        ...
        if (saveDataCheck()) {
           ...
           getCoderData(theCodingData.pageId+lastDocPos, MINUS_ONE, B_CHILD);            
        }
    }                                             
    ...
    private void clear() {  // Alt+X
        //Log.print("(QAProofReadingpage).clearButtonActionPerformed");
        if (unitize) {
            saveAndGo = true;
            boundaryText.setText("");
            //saveAndGoButton.doClick();
    ...
    private void child() {  // Alt+I
        //Log.print("(QAProofReadingpage).childButtonActionPerformed");
        if (unitize) {
            saveAndGo = true;
            boundaryText.setText("C");
            //saveAndGoButton.doClick();
    ...
    private void doc() {   // Alt+D
        //Log.print("(QAProofReadingpage).docButtonActionPerformed");
        if (unitize) {
            saveAndGo = true;
    ...
    //private void batch() {
    //    Log.print("(QAProofReadingpage).batchButtonActionPerformed");
    //    if (unitize) {
    //        batchingFlag = true;
    //        boundaryText.setText("D");
    //        saveAndGoButton.doClick();
                ...
                try {
                    ...
                    if (result instanceof String) {
                        if ( ((String)result).equals(T_OK)) {
                            ...
                            closeBatchMenuItem.setEnabled(true);
                            viewerLabel.setFont(new Font("Dialog", Font.PLAIN, 14));
                            //Log.print("QAProofReadingpage.validateBatch returned notOK error ");
                        }
                        ...
                        closeBatchMenuItem.setEnabled(true);
                        viewerLabel.setFont(new Font("Dialog", Font.PLAIN, 14));
                        //Log.print("QAProofReadingpage.validateBatch returned error ");
                    }
                } catch (Throwable th) {
                    Log.quit(th);
                }
        ...
        task.setCallback(new Runnable() {
                public void run() {
                    try {
                        ...
                        String batchName = QAProofReadingpage.this.projectName
                                + " Batch " + QAProofReadingpage.this.theCodingData.batchNumber;
                        assert (! "QA".equals(whichStatus));
                        ...
                        QAProofReadingpage.this.closeViewer();
                    } catch (Throwable th) {
                        Log.quit(th);
                    }
                }
    ...
    protected void getCoderData(int pageId, int offset, int boundary) {
        ...
        try {
            ...
            final ClientTask task = new TaskRequestCoding(pageId, offset, boundary, "Coding");
            setCoderCallback(task);
        } catch (Throwable th) {
            Log.quit(th);
        }
...
//            final ClientTask task = new TaskRequestCoding(pageId, offset, boundary, whichStatus,bateno,volumeId,flag);
    ...
    public void codingEntry(CodingData codingData) {
        if (codingData == null) {
            Log.write("QAProofReadingpage.codingEntry codingData is null");
            return;
        }
       // Log.print("(QAProofReadingpage).codingEntry " + codingData.pageId + " ProjectMap " + projectMap.size());
...
//       if(true){
//           new beans.LTextField(codingData);
//       }  
             
        //set the image for F10 added by bala
        if (null != area) {
        ...
        if("Yes".equals(codingData.query_raised)){     
        ...
        //Log.print("(QAProofReadingpage).codingEntry back from viewerEntry");
    ...
    public void viewerEntry(ImageData data) {            
        if (data == null) {
            Log.write("QAProofReadingpage.viewerEntry data is null");
            return;
        }
        //batch_number = data.documentNumber;
        //System.out.println("documentNumber---------------->" + documentNumber);
        Log.print("(QAProofReadingpage).viewerEntry "
                  +" "+data.imagePath + "/" + data.path + "/" + data.filename+" "
                  + Integer.toHexString(data.boundaryInfo));
        theImageData = data;
        ...
        if (unitize) {
            if (! boundaryFromServer
                && theCodingData.pageId == theImageData.pageId) {
                // put the boundary flag on the screen
                fromServer = true;
                //Log.print("(QAProofReadingpage).viewerEntry set boundaryText");
        ...
        viewerLabel.setText(" " + theImageData.batesNumber
                         + " (" + theImageData.childImagePosition
                         + " of " + theImageData.childImageCount + ")");
        ...
        if (theImageData.childId != theCodingData.childId) {
            ...
            if (unitize
                && ! theCodingData.firstBatesOfChild.equals(theImageData.batesNumber)) {
        ...
        //Log.print("QAProofReadingpage.viewerEntry set buttons " + boundaryText.getText());
        if (! unitize) {
            ...
            if ("Binder".equals(whichStatus)
                || isBoundary(theImageData, LAST_RANGE_OF_VOLUME))
            {
                vAttachNextButton.setEnabled(false);
            } else {
            ...
            if ("Binder".equals(whichStatus)
                || isBoundary(theImageData, FIRST_RANGE_OF_VOLUME))
            {
                vAttachPrevButton.setEnabled(false);
            } else {
                //Log.print("enable vDocPrevButton");
        ...
        Log.print("QAProofReadingpage.viewerEntry after set buttons " + boundaryText.getText());        
    }
    ...
    protected void sendCodingData(int pageId, Map valueMap) {
        ...
        final ClientTask task = new TaskSendCodingData(pageId
                                                       , boundaryFlag
                                                       , valueMap
                                                       , theCodingData.valueMap
                                                       , errorMap
                                                       ,errorTypeMap,whichStatus);
        ...
        if (go) { // User clicked Save & Go
            if (unitize
                ...
                if (isBoundary(theCodingData, LAST_CHILD_OF_BATCH)) {
                    ...
                    getCoderData(theCodingData.pageId, PLUS_ONE
                                 , ("QA".equals(whichStatus) ? B_UNCODED : B_CHILD));
                }
            }
        } else {
            if (unitize
    ...
    protected void getPageValues(int pageId, int offset) {
        final ClientTask task = new TaskRequestCodingValues(pageId
                                            , offset, B_CHILD);
    ...
    private boolean doViewFromSource(ImageData data) {
        ...
        if (activeGroup == 1) {           
            ...
            Log.print("(SPV.doViewFromSource) " + data.imagePath + "/" + data.groupOnePath
                        + "/" + data.groupOneFilename);
            filename = data.imagePath + "/" + data.groupOnePath
                        + "/" + data.groupOneFilename;
        } else {               
            filename = data.imagePath + "/" + data.path + "/" + data.documentNumber
                        + "/" + data.filename;
        }      
    ...
    void Constrain(java.awt.Container container, java.awt.Component component, int gridx, int gridy, int gridwidth, int gridheight, int fill, int anchor, int weightx, int weighty, int top, int left, int bottom, int right)
    {
            ...
            if (gridx == -1) gridx = gridXPos;
            if (gridy == -1) gridy = gridYPos;
            ...
            if (weightx == 0)
                    c.weightx = 0.0;
            else c.weightx = weightx;
            if (weighty == 0)
                    c.weighty = 0.0;
            else c.weighty = weighty;
    ...
    public boolean saveDataCheck() {
        ...
        if (changed == null && unitize
            && ! ((boundaryText.getText()).toUpperCase())
                   .equals(theCodingData.boundaryFlag))
        {
...
*/
    ...
    */
    ...
    private void copyField() {
        ...
        copyField = (JComponent)KeyboardFocusManager
     ...
     */
    private void save() {
        Log.print("(SPV.save) coding/image " + theCodingData.pageId + "/"
                  + theImageData.pageId);
        checkCloseAllDialogs();
    ...
    //        Toolkit.getDefaultToolkit().beep();
    //        Log.print("BEEP> QAProofReadingpage.deleteDialog");
    //        Object[] options = {"Yes"
    //                          , "No"};
    ...
    protected Action nextDocAction = new AbstractAction()
        {
            public void actionPerformed(java.awt.event.ActionEvent A) {
                //Log.print("(QAProofReadingpage).nextDocAction event is RIGHT ");
                //if (unitize){
                //    uPageNext();
                //} else {
                    docNext();
    ...
    protected Action nextDocPageAction = new AbstractAction()
        {
            public void actionPerformed(java.awt.event.ActionEvent A) {
                //Log.print("(QAProofReadingpage).nextDocBoundaryAction event is RIGHT ");
                uPageNext();
                //docNext();
            }
        };
    ...
    protected Action homeAction = new AbstractAction()
        {
            public void actionPerformed(java.awt.event.ActionEvent A) {
                Log.print("(QAProofReadingpage).homeAction ");
                if (unitize) {
                    boundaryText.requestFocus();
                } else {
                    projectMap.getComponent(0).requestFocus();
    ...
    protected Action prevDocAction = new AbstractAction()
        {
            public void actionPerformed(java.awt.event.ActionEvent A) {
                //Log.print("(QAProofReadingpage).prevDocAction event is LEFT ");
                //if (unitize){
                //    uPagePrev();
                //} else {
                    docPrev();
    ...
    */
    protected Action prevDocPageAction = new AbstractAction()
        {
            public void actionPerformed(java.awt.event.ActionEvent A) {
                //Log.print("(QAProofReadingpage).prevDocBoundaryAction event is LEFT ");
                uPagePrev();
                //docPrev();
            }
        };
    ...
    private void checkCloseDialog(Component comp) {
        ...
        if (comp != null
            && comp.getParent() instanceof beans.LTextButton) {
            ((beans.LTextButton)comp.getParent()).closeDialog();
        } else if (comp != null
                   && comp instanceof beans.LTextButton) {
     ...
     */
        private void hideCurrentPopup() {
            ...
            if (currentFocus != null
                && currentFocus.getParent() instanceof IbaseTextField)
            {
     ...
     */
        private void resetInputVerifier() {
            Component currentFocus = KeyboardFocusManager.getCurrentKeyboardFocusManager().getFocusOwner();
            if (currentFocus != null
                && currentFocus.getParent() != null
                && (currentFocus.getParent() instanceof LTextField
                || currentFocus.getParent() instanceof beans.LDateField) ) {
	...
	private class ProjectFieldsTableClass extends JTable {
     ...
     private class ProjectDocumentTableClass extends JTable {
                public String getToolTipText(MouseEvent event) {
                    return ToolTipText.getToolTipText(event, docsTable);
                }
                public Point getToolTipLocation(MouseEvent event) {
                    return ToolTipText.getToolTipLocation(event, docsTable);
                }
            }

    // Variables declaration - do not modify                     
    private javax.swing.JMenuItem AboutMenuItem;
    private javax.swing.JMenuItem closeQAProofReading;
    ...
    public javax.swing.JPanel viewerPane;
    private javax.swing.JToolBar viewerToolBar;

    private javax.swing.JPanel docsPane;
    private javax.swing.JLabel docsLabel;
    private javax.swing.JLabel sampleFieldsLabel;
    private javax.swing.JTextArea sampleFieldsTextArea;
    ...
    private javax.swing.JPanel sampleFieldsPane;
    private javax.swing.JPanel sampleBRPane;
    private javax.swing.JButton saveCurrentDocButton;
    private javax.swing.JButton deleteDocButton;
    private javax.swing.JButton clearDocButton;
    private javax.swing.JPanel samplingButtonsPane;
    private javax.swing.JPanel correctionPane;
    private javax.swing.JPanel correctionDataPane;
    private javax.swing.JScrollPane correctionScrollPane;
    ...
    private javax.swing.JScrollPane docsScrollPane;
    ...
    private javax.swing.JLabel correctionDataLabel;
...
//	private javax.swing.JTextField newValuetextfield;
    ...
    private javax.swing.JMenuItem count;
	 ...
	 //---------------->
		//-----------coded anurag -------------------
		public javax.swing.JPanel belowPanel;
     ...
     */
    public void setParent(QAGroupAssignWindow Parent) {
        parent = Parent;
    }
    ...
    private void closeViewer() {
        ...
        if ("Binder".equals(whichStatus) && parent != null) {
            //((QAProofReadingpage) parent).binderViewer = null;
        }
        ...
        /*if (Global.mainWindow == this) {
            ...
            if("Listing".equals(whichStatus)|| "ListingQC".equals(whichStatus)
                    ||"Tally".equals(whichStatus) || "TallyQC".equals(whichStatus)){
    ...
    public void addTimerPanel(String status){
    ...
    private void showBreakDialog(){
        System.out.println("Show Break Dialog");
    ...
    public void createBreakDialog(int event_Break_Id){
     ...
     JDialog breakDialog = new ShowBreakDialog(this,"Break",event_Break_Id,duration,"QAProofReading",userId);
    ...
    private class FocusComponent implements FocusListener {
        public void focusGained(FocusEvent e)
        {
            ...
            if (! rectViewport.contains(new Point(0, (int) rect.getY()))
                || ! rectViewport.contains(new Point(0, (int) rect.getY() + 20))) {
     ...
     * on whatever QAProofReadingpage contains the control
     * @param location The control which wants to clear viewer.
     */
    public static void clearViewerStatus(Component location) {
        if (location instanceof QAProofReadingpage) {
            QAProofReadingpage viewer = (QAProofReadingpage) location;
   ...
   public void setFieldType(String fieldType)
   {
    ...
    //newlly implemented methods
    
    private void fillDocumentTable() {
        
        if(null != sampledDocumentList) {
            String headings[] = {"Document No", "First Page", "End Page"};
            docTableObj = new Object[sampledDocumentList.size()][headings.length + 4];
            Set keySet = sampledDocumentList.keySet();
            Iterator it = keySet.iterator();
            int i = 0;
            while(it.hasNext()) {
                List valueList = (List)sampledDocumentList.get(it.next());
                docTableObj[i][0] = valueList.get(0);
                docTableObj[i][1] = valueList.get(1);
                docTableObj[i][2] = valueList.get(2);
                docTableObj[i][3] = valueList.get(3);
                docTableObj[i][4] = valueList.get(4);
                docTableObj[i][5] = valueList.get(5);
                i++; 
            }

             qamodel = new QAGroupModel(docTableObj , headings);
             docsTable.setModel(qamodel);    
            }  
    }
    
    private void docsTableMouseClicked(boolean isDisplayImage) {
        
        //when ever the user selects new document disable all the buttons
        saveCurrentDocButton.setEnabled(false);
        deleteDocButton.setEnabled(false);
        clearDocButton.setEnabled(false);
        correctionTextArea.setEnabled(false);
        int selectedRow = docsTable.getSelectedRow();
        int selectedDocChildId = Integer.parseInt(docsTable.getValueAt(selectedRow, 0).toString());
        String startBatesNumber = docsTable.getValueAt(selectedRow, 1).toString();
        int samplingId = Integer.parseInt(docTableObj[selectedRow][4].toString());
        sampleFieldsTextArea.setText("");
        correctionTextArea.setText("");
        sampleFieldsLabel.setText("SAMPLED FIELD");
        final ClientTask task;
        task = new TaskGetQAFieldValue(this.projectId, this.volumeId, selectedDocChildId, samplingId, samplingType);
        task.setCallback(new Runnable() {

                    public void run() {
                       loadFieldValueTable((Map)task.getResult());
                   }
                });
        task.enqueue(this);
        
        if(isDisplayImage) {
            displayImageForDocument(startBatesNumber, selectedDocChildId,"l2");
        }
    ...
    private void loadFieldValueTable(Map fieldValueList) {
        
        if(null != fieldValueList) {
            int selectedDoc = docsTable.getSelectedRow();
            String sampledFields = docTableObj[selectedDoc][5].toString();
            
            final String headings[] = {"FieldName", "FieldData", "CorrectionData","CorrectionType"};
            summaryTableObj = new Object[fieldValueList.size()][headings.length + 8];
            Set splitedValueKeySet = fieldValueList.keySet();
            Iterator it = splitedValueKeySet.iterator();
            int i = 0;
            Object l1ChildId;
            Object startBates;
            while(it.hasNext()) {
                ...
                correctionData = "";
                correctionType = "";
                List splitedValueList = (List)fieldValueList.get(it.next());
                
                    fieldName = splitedValueList.get(0).toString();
                    Object indexObj = splitedValueList.get(3);
                    Object fieldValueObj = splitedValueList.get(1);
                    Object fieldId = splitedValueList.get(2);
                    System.out.println("sampledFields :" +sampledFields);
                    String fieldValue = "";
                    correctionId = Integer.parseInt(splitedValueList.get(4).toString());
                    correctionData = splitedValueList.get(5).toString();
                    correctionType = splitedValueList.get(6).toString();
                    l1ChildId = splitedValueList.get(8).toString();
                    startBates = splitedValueList.get(9).toString();                    
                    int valueIndex = 0;
                    if(null != sampledFields && sampledFields.equals("ALL")) {
                        if(null != fieldValueObj) {
                            fieldValue = fieldValueObj.toString();

                            summaryTableObj[i][0] = fieldName;
                            summaryTableObj[i][1] = fieldValue;
                            summaryTableObj[i][2] = correctionData;
                            summaryTableObj[i][3] = correctionType;
                            summaryTableObj[i][4] = fieldId;
                            summaryTableObj[i][5] = indexObj;
                            summaryTableObj[i][6] = correctionId;
                            summaryTableObj[i][7] = l1ChildId;
                            summaryTableObj[i][8] = startBates;
                            summaryTableObj[i][9] = splitedValueList.get(10);
                            summaryTableObj[i][10] = "false";
                            summaryTableObj[i][11] = splitedValueList.get(11);                            
                            i++; 
                        }
                    }else {
                        String[] sampledFieldsArr = sampledFields.split(",");
                        boolean isFieldExists = false;
                        for(int k=0; k<sampledFieldsArr.length; k++) {
                            if(fieldId.toString().equals(sampledFieldsArr[k])) {
                                isFieldExists = true;
                                break;
                            }
                        }
                        if(null != fieldValueObj && isFieldExists) {
                            fieldValue = fieldValueObj.toString();
            ...
            qamodel = new QAGroupModel(summaryTableObj , headings);
            summaryTable.setModel(qamodel);    
    ...
    private void summaryTableMouseClicked(java.awt.event.MouseEvent evt) {
        int selectedFieldValue = summaryTable.getSelectedRow();
        String startBate = "";
        int l1ChildId = 0;
        
        String showField = summaryTableObj[selectedFieldValue][10].toString();
        fieldType = summaryTableObj[selectedFieldValue][11].toString();
        System.out.println("showField:"+showField);
        
        if(null != showField && showField.equals("true")) {
            ...
            String fieldName = summaryTable.getValueAt(selectedFieldValue, 0).toString();
            String fieldValue = summaryTable.getValueAt(selectedFieldValue, 1).toString();
            String correctionData = summaryTable.getValueAt(selectedFieldValue, 2).toString();
            String correctionType = summaryTable.getValueAt(selectedFieldValue, 3).toString();
            String fieldLevel = summaryTableObj[selectedFieldValue][9].toString();
            sampleFieldsTextArea.setText(fieldValue);
            correctionTextArea.setText(correctionData);
            saveCurrentDocButton.setEnabled(true);
            if((null != correctionData && !correctionData.isEmpty()) || (null != correctionType && !correctionType.isEmpty())) {
                deleteDocButton.setEnabled(true);
            }else {
                deleteDocButton.setEnabled(false);
            }
            clearDocButton.setEnabled(true);
            correctionTextArea.setEnabled(true);            
            sampleFieldsLabel.setText("SAMPLED FIELD : "+fieldName);
            if("l1".equals(fieldLevel)) {
                startBate = summaryTableObj[selectedFieldValue][8].toString();
                l1ChildId = Integer.parseInt(summaryTableObj[selectedFieldValue][7].toString());
                displayImageForDocument(startBate, l1ChildId,fieldLevel);
            }else {
                int selectedDocIndex = docsTable.getSelectedRow();
                startBate = docsTable.getValueAt(selectedFieldValue,1).toString();
                l1ChildId = Integer.parseInt(docsTable.getValueAt(selectedFieldValue,0).toString());
                displayImageForDocument(startBate, l1ChildId,fieldLevel);
            }
        }
    ...
    private void saveCurrentDocButtonActionPerformed(java.awt.event.ActionEvent evt) {
        int selectedDoc = docsTable.getSelectedRow();
        int selectedValue = summaryTable.getSelectedRow();
        Date date = new Date();
        long time = date.getTime();
        Timestamp timestamp = new Timestamp(time);
        
        if(fieldType.equals(CommonConstants.FIELD_TYPE_DATE)){
                if(!verifyDate(correctionTextArea)){
                    JOptionPane.showMessageDialog(this.correctionTextArea, "Enter a valid date","Date Format :  YYYYMMDD", JOptionPane.ERROR_MESSAGE);
                    return;
                }
        }
        int docId = Integer.parseInt(docTableObj[selectedDoc][3].toString());
        int childId = Integer.parseInt(docTableObj[selectedDoc][0].toString());
        int fieldId = Integer.parseInt(summaryTableObj[selectedValue][4].toString());
        int tagSeq = Integer.parseInt(summaryTableObj[selectedValue][5].toString());
        String fieldName = summaryTable.getValueAt(selectedValue, 0).toString();
        String fieldValue = summaryTable.getValueAt(selectedValue, 1).toString();
        String correctionValue = correctionTextArea.getText().trim();
        String correctionType = correctionComboBox.getSelectedItem().toString();
        int samplingId = Integer.parseInt(docTableObj[selectedDoc][4].toString());
        
        final ClientTask task;
        task = new TaskSaveQACorrectionData(docId, fieldId, childId, tagSeq,fieldValue, correctionValue, correctionType, ""+fieldValue,samplingId);
        task.setCallback(new Runnable() {

                    public void run() {
                       docsTableMouseClicked(false);
                   }
                });
        task.enqueue(this);
    ...
    private void deleteDocButtonActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        int confirmResult = JOptionPane.showConfirmDialog(this, "Are you sure, do you want to delete this entry ?","Please Confirm" ,JOptionPane.YES_NO_OPTION);

        if(confirmResult == 0) {
            int selectedValue = summaryTable.getSelectedRow();
            int correctionId = Integer.parseInt(summaryTableObj[selectedValue][6].toString());

            if(correctionId != 0 && correctionId != -1) {
                final ClientTask task;
                task = new TaskExecuteUpdate("qapr.deleteCorrections",""+correctionId);
                task.setCallback(new Runnable() {
    ...
    private void clearDocButtonActionPerformed(java.awt.event.ActionEvent evt) {
        correctionTextArea.setText("");
       
    }
    
    private boolean verifyDate(JComponent input) {
        Calendar theCalendar = new GregorianCalendar();
        SimpleDateFormat formatter = new SimpleDateFormat("yyyyMMdd");
        SimpleDateFormat mmddformatter = new SimpleDateFormat("MMdd");
