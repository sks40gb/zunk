...
public class CsvWriter implements MessageConstants {
     ...
     */
    public CsvWriter(String filename, ProjectFieldsData[] fields, String field_delimiter, String text_qualifier, String valueSeparator, String dateFormat, RunStats stats) throws IOException {
        // for writeLog
        this(filename, fields, null, field_delimiter, text_qualifier, valueSeparator, null, null, null, dateFormat, false, false, null, null, null, null, false, false, false, stats);
    }

    public CsvWriter(String filename, ProjectFieldsData[] fields, PageissueData[] pageissues, String fieldDelimiter, String textQualifier, String valueSeparator, String missingYear, String missingMonth, String missingDay, String dateFormat, boolean uppercase, boolean uppercase_names, String name_mask1, String name_mask2, String name_mask3, String name_mask4, boolean unitizeOnly, boolean nullAttachments, boolean brsFormat, RunStats stats //, boolean first_name_initial, boolean include_mi
            ) throws IOException {
        ...
        if (dateFormat != null && missingDay != null) {
    ...
    public void writeValue(int pageIndex, ValueData text, ProjectFieldsData field) {
...
//        }
        ...
        if (childLft == -1) { // first write
            ...
            return;
        } else if (childLft == text.childLft && lastField.sequence == field.sequence && text.sequence == valueSequence) {
    ...
    private void putLeadingDocs(int pageIndex) {
        ...
        if (brsFormat) {
            ...
            buffer.append(textQualifier + leadingBates + textQualifier + fieldDelimiter);
        }
        ...
        for (i = pageIndex + 1; i < pages.length; i++) {
            if (pages[i].lft > pages[pageIndex].lft) {
                ...
                buffer.append(textQualifier + endBates + textQualifier + fieldDelimiter);
                break;
            }
        }
        ...
        //}
        if (!brsFormat) {
            if (nullAttachments && leadingBates.equals(pages[pageIndex].beginBatesNumber) && endBates.equals(pages[pageIndex].endBatesNumber)) {
                // leave the attachment range empty if the doc is not
                // part of a range
                buffer.append(textQualifier + textQualifier + fieldDelimiter);
                buffer.append(textQualifier + textQualifier);
            } else {
                // repeat the doc bates for attachment range if the doc is not
                // part of a range
                buffer.append(textQualifier + pages[pageIndex].beginBatesNumber + textQualifier + fieldDelimiter);
                buffer.append(textQualifier + pages[pageIndex].endBatesNumber + textQualifier);
            }
        } else {
            writeBuffer();
            fieldDelimiter("ATTR");
            if (leadingBates.equals(pages[pageIndex].beginBatesNumber) && endBates.equals(pages[pageIndex].endBatesNumber)) {
            } else {
    ...
    private String nameValue(String name) {
        ...
        if (name_mask1.length() > 0 || name_mask2.length() > 0 || name_mask3.length() > 0 || name_mask4.length() > 0) {
            int i = 0;
            // parse the name
            String last = "";
            String first = "";
            ...
            if (middle.length() < 1 && last.length() < 1 && affiliation.length() < 1) {
                // TBD: Specific to BRS?
            ...
            text = "";
            text = getMaskedName(text, name_mask1, first, last, middle, affiliation);
            text = getMaskedName(text, name_mask2, first, last, middle, affiliation);
            text = getMaskedName(text, name_mask3, first, last, middle, affiliation);
            text = getMaskedName(text, name_mask4, first, last, middle, affiliation);
            return text;
        }
     ...
     */
    private String getMaskedName(String text, String mask, String first, String last, String middle, String affiliation) {
        if (mask.length() <= 0) {
    ...
    private String upperValue(String type, String value) {
        ...
        if (type.equals("name") && uppercase_names) {
            return value.toUpperCase();
        }
        return value;
    }
    ...
    private String dateValue(String date) {
        ...
        if (date.equals("00000000")) {
            ...
            if (date.substring(6).equals("00") || date.length() > 8) {
                //Log.print("  empty day");
                // empty day
                for (int i = 0; i < 30; i++) {
    ...
    public void writeLog(String project_name) {
        ...
        writer.println(" ");
        writer.println(" ");
        writer.println(project_name + "  " + stats.recordCount + " records/" + stats.pageCount + " pages");
        if (fields != null) {
            ...
            writer.println(" ");
            writer.println("Delimiters are " + fieldDelimiter + textQualifier + " and the date is formatted " + dateFormat.toLowerCase() + ".");
        }
    }
